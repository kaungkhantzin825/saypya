...
@section('title', 'ရှာဖွေမှုရလဒ်များ')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            @if($query)
                <h1 class="text-3xl font-bold text-gray-900 mb-2 myanmar-text">
                    "{{ $query }}" အတွက် ရှာဖွေမှုရလဒ်များ
                </h1>
                <p class="text-gray-600 myanmar-text">{{ $courses->total() }} သင်ခန်းစာတွေ့ရှိသည်</p>
            @else
                <h1 class="text-3xl font-bold text-gray-900 mb-2 myanmar-text">သင်ခန်းစာများ</h1>
                <p class="text-gray-600 myanmar-text">{{ $courses->total() }} သင်ခန်းစာရရှိနိုင်သည်</p>
            @endif
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
            <form method="GET" action="{{ route('search') }}" class="space-y-4">
                <!-- Search Input -->
                <div>
                    <label for="search" class="block text-sm font-medium text-gray-700 mb-2 myanmar-text">ရှာဖွေရန်</label>
                    <input type="text" name="q" id="search" value="{{ $query }}" 
                           placeholder="သင်ခန်းစာများကို ရှာဖွေပါ..." 
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500 myanmar-text">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="sort" id="sort" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">
                            <option value="relevance" {{ request('sort') == 'relevance' ? 'selected' : '' }} class="myanmar-text">သက်ဆိုင်မှု</option>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
            ...
            <div class="col-span-full text-center py-12">
                ...
                <p class="text-gray-600 mb-4 myanmar-text">
                    @if($query)
                        "{{ $query }}" အတွက် ရှာဖွေမှုရလဒ်များ မတွေ့ရှိပါ။
                    @else
                        ရှာဖွေမှုစံနှုန်းများနှင့် ကိုက်ညီသော သင်ခန်းစာများ မရှိပါ။
                    @endif
                </p>
                <a href="{{ route('courses.index') }}" class="text-blue-600 hover:text-blue-700 font-semibold myanmar-text">
                    သင်ခန်းစာများအားလုံးကြည့်ရန်
                </a>
            </div>
            @endforelse
        </div>
